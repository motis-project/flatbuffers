...
static std::string GenTypePointer(const Parser &parser, const Type &type) {
  switch (type.base_type) {
    case BASE_TYPE_STRING:
      return FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::String";
    case BASE_TYPE_VECTOR:
      return FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::Vector<" +
...
static std::string GenTypeWire(const Parser &parser, const Type &type,
  ...
  return IsScalar(type.base_type)
    ...
    : IsStruct(type)
      ...
      : FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::Offset<" + GenTypePointer(parser, type) + ">" + postfix;
}
...
static std::string GenTypeSize(const Parser &parser, const Type &type) {
  return IsScalar(type.base_type)
    ...
    : IsStruct(type)
      ...
      : FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::uoffset_t";
}
...
std::string EnumSignature(EnumDef &enum_def) {
  return "inline bool Verify" + enum_def.name +
         "(" FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::Verifier &verifier, " +
...
static void GenTable(const Parser &parser, StructDef &struct_def,
  ...
  code += " FLATBUFFERS_FINAL_CLASS : private " FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::Table";
  code += " {\n";
  ...
  for (auto it = struct_def.fields.vec.begin();
    ...
    if (!field.deprecated) {  // Deprecated fields won't be accessible.
      ...
      if (nested) {
        ...
        code += "_nested_root() const { return " FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::GetRoot<";
  ...
  code += "  bool Verify(" FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::Verifier &verifier) const {\n";
  ...
  code += "struct " + struct_def.name;
  code += "Builder {\n  " FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::FlatBufferBuilder &fbb_;\n";
  code += "  " FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::uoffset_t start_;\n";
  for (auto it = struct_def.fields.vec.begin();
       it != struct_def.fields.vec.end();
       ++it) {
    auto &field = **it;
  ...
  code += "Builder(" FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::FlatBufferBuilder &_fbb) : fbb_(_fbb) ";
  code += "{ start_ = fbb_.StartTable(); }\n";
  code += "  " + struct_def.name + "Builder &operator=(const ";
  code += struct_def.name + "Builder &);\n";
  code += "  " FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::Offset<" + struct_def.name;
  code += "> Finish() {\n    auto o = " FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::Offset<" + struct_def.name;
  ...
  code += "inline " FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::Offset<" + struct_def.name + "> Create";
  code += struct_def.name;
  code += "(" FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::FlatBufferBuilder &_fbb";
  for (auto it = struct_def.fields.vec.begin();
       it != struct_def.fields.vec.end();
       ++it) {
    auto &field = **it;
...
static void GenStruct(const Parser &parser, StructDef &struct_def,
  ...
  for (auto it = struct_def.fields.vec.begin();
    ...
    code += field.name + "_(";
    if (IsScalar(field.value.type.base_type)) {
      code += FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::EndianScalar(";
      code += GenUnderlyingCast(field, false, "_" + field.name);
      code += "))";
    } else {
      code += "_" + field.name + ")";
  ...
  // type name() const { return FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) ::EndianScalar(name_); }
  for (auto it = struct_def.fields.vec.begin();
       it != struct_def.fields.vec.end();
       ++it) {
    auto &field = **it;
    ...
    code += GenUnderlyingCast(field, true,
      is_scalar
        ? FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::EndianScalar(" + field.name + "_)"
        : field.name + "_");
    code += "; }\n";
    if (parser.opts.mutable_buffer) {
      if (is_scalar) {
        ...
        code += GenTypeBasic(field.value.type, true);
        code += " _" + field.name + ") { " FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::WriteScalar(&";
        code += field.name + "_, ";
        code += GenUnderlyingCast(field, false, "_" + field.name);
        code += "); }\n";
      } else {
  ...
  bool generate() {
    ...
    if (parser_.opts.include_dependence_headers) {
      ...
      for (auto it = parser_.included_files_.begin();
        ...
        auto basename = StripPath(StripExtension(it->first));
    ...
    if (parser_.root_struct_def_) {
      ...
      code += name;
      code += "(const void *buf) { return " FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::GetRoot<";
      code += cpp_qualified_name + ">(buf); }\n\n";
      if (parser_.opts.mutable_buffer) {
        code += "inline " + name + " *GetMutable";
        code += name;
        code += "(void *buf) { return " FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::GetMutableRoot<";
        code += name + ">(buf); }\n\n";
      }
      ...
      code += name;
      code +=
          "Buffer(" FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::Verifier &verifier) { "
      ...
      if (parser_.file_identifier_.length()) {
        ...
        code += "BufferHasIdentifier(const void *buf) { return "  FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::";
      ...
      code +=
          "Buffer(" FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::FlatBufferBuilder &fbb, " FLATBUFFERS_STRING(FLATBUFFERS_NAMESPACE) "::Offset<";
